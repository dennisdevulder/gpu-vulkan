...
## 1. Sidebar collapse arrow crashes the EDT (RELEASE BLOCKER)

### Symptom
Plugin runs fine. User clicks the chevron/arrow that collapses the RuneLite
plugin sidebar. RuneLite window vanishes (process exits).
...
- `awtContext.createGLContext()` (left bound) — same crash.
...
### Real fix path
**Vulkan-to-offscreen + GL blit** (~1-2 days). Render Vulkan to an offscreen
image, share it to GL via `VK_KHR_external_memory` + `GL_EXT_memory_object`,
GL renders the texture to canvas via `swapBuffers`. AWT sees a normal GL
canvas, layout changes safe. This is the architecture every other
Vulkan-on-AWT project uses (LWJGL3 demos, Avian, etc.).

### Workaround until then
Document in plugin description: "Don't collapse the side panel while plugin
is active. Toggle the plugin off first."
